...
Environment for CenterPoint and BEVFusion inference (mmdetection3d 1.4.0). This is the **verified working recipe**, reconstructed after resolving a long chain of version conflicts. Rebuilds identically on Vast.ai — only the torch line changes (CPU locally → CUDA on the GPU box).
